...
from auth import BASE_BACKUP_FOLDER, SHAREPOINT_DRIVE_ID, get_graph_client
from download_graph import _download_folder_recursive, _list_children
...
async def download_summary(client, dataset_config, summary_name):
    ...
    try:
        ...
        if (
            ...
            and e.error.code == "itemNotFound"
        ):
            raise FileNotFoundError(
                f"Folder {dataset_config['folder_path']} not found in dataset drive."
            )
    ...
    summary_item = next(
        (item for item in items.value if item.name == summary_name), None
    )
    if not summary_item:
        raise FileNotFoundError(
            f"{summary_name} not found in {dataset_config['folder_path']}."
        )
...
def get_latest_software_version(json_file):
    if not json_file.exists():
        return V("0.0.0")
    ...
    for img in data.get("images", []):
        if not img.get("annotations"):
            continue
        raw = img.get("app_version", "0.0.0")
        base = raw.split("-", 1)[0]  # "2.3.0-bbc3e33" → "2.3.0"
        if base == "0.0.0":
            continue
        try:
    ...
    return max(versions) if versions else V("0.0.0")
...
async def run_study_report(client, study, dataset_config, skip_missing=False):
    ...
    print(
        f"{'Annotator':<10} | {'Group':<10} | {'Progress':<10} | {'Images (Ann/Total)':<16} | {'SW Version'}"
    )
    print("-" * 70)
    ...
    for group_id, group_info in group_mapping.items():
        ...
        latest_sw_version = get_latest_software_version(file_path)
        group_size = final_group_sizes.get(group_id, 0)

        if group_size > 0:
            percentage = (annotated / group_size) * 100
            print(
                f"{annotator:<10} | {group_id:<10} | {percentage:>8.2f}%  | {annotated:>4}/{group_size:<4}          | {latest_sw_version}"
            )
    ...
    if total_expected > 0:
        ...
        print(
            f"{'STUDY TOTAL':<10} | {'':<10}| {percentage:>8.2f}%  | {total_annotated}/{total_expected}"
        )
    print()
...
async def main():
    ...
    if args.study:
        ...
        if not study:
            ...
            return
        await run_study_report(
            client, study, datasets[study["dataset_id"]], skip_missing=args.skip_missing
        )
    else:
        for study in studies:
            a, e = await run_study_report(
                client,
                study,
                datasets[study["dataset_id"]],
        ...
        if global_expected > 0:
            ...
            print("-" * 70)
            print(
                f"{'GLOBAL':<10} | {'':<10} | {percentage:6.2f}% | {global_annotated}/{global_expected}"
            )
